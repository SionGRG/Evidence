...
PlayMode::PlayMode(MyD3D & d3d)
	:mD3D(d3d), mPlayer(d3d), mThrust(d3d)
{
	InitBgnd();
	InitPlayer();
}
...
{
	mBulletRelTimer += dTime;
	if (Game::sMKIn.IsPressed(VK_SPACE))
	{
		//find a sleeping one
		int i = 0;
		while (i < mMissiles.size() && mMissiles[i]->active)
			++i;
		//did we get one?
		if (i < mMissiles.size())
		{
			mBulletRelTimer = 0;
			mMissiles[i]->active = true;
			mMissiles[i]->spr.mPos = Vector2(mPlayer.mPos.x + mPlayer.GetScreenSize().x / 2.f, mPlayer.mPos.y);
		}
	}

	for (int i = 0; i < mMissiles.size(); i++)
		mMissiles[i]->Update(dTime);
}
...
void PlayMode::InitPlayer()
{
	...
	for (int i = 0; i < 50; i++)
	{
		Bullet*p = new Bullet (mD3D);
		mMissiles.push_back(p);
	}
}
